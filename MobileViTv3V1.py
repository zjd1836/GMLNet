...
class MobileViTv3_v1(nn.Module):
    def __init__(self, image_size, mode, isThree=False, patch_size=(2, 2)):
        """
        ...
        self.layer_3 = nn.Sequential(
            ...
            MobileViTBlockV3_v1(channels[3], attn_dim[0], ffn_multiplier, heads=4, dim_head=8, attn_blocks=2,
                                patch_size=patch_size)
        )
        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[3], channels[4], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[4], attn_dim[1], ffn_multiplier, heads=4, dim_head=8, attn_blocks=4,
                                patch_size=patch_size)
        )
        self.layer_5 = nn.Sequential(
            InvertedResidual(channels[4], channels[5], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[5], attn_dim[2], ffn_multiplier, heads=4, dim_head=8, attn_blocks=3,
                                patch_size=patch_size)
        )
    ...
    def load_pretrained_model(self, ckpt_path):
        ...
        for key1 in state_dict:
            keys1_name.append(key1)
        for key2 in pretrain_dict:
            keys2_name.append(key2)
        keys2_name = keys2_name[:303]
        for item1, item2 in zip(keys1_name, keys2_name):
            model_dict[item1] = pretrain_dict[item2]
        ckpt = OrderedDict(model_dict)
        self.load_state_dict(ckpt)


    def forward(self, x):
        x = self.conv_1(x)
        x = self.layer_1(x)  # 32*255*255
        x = self.layer_2(x)  # 64*128*128
        x3 = self.layer_3(x)  # 128*64*64
        x4 = self.layer_4(x3)  # 256*32*32
        out = self.layer_5(x4)  # 320*16*16
        if self.isThree:
            return [x3, x4, out]
        else:
            return [x, x3, x4, out]
...
if __name__ == '__main__':
    ...
    model = MobileViTv3_v1(
        image_size=(512, 512),
        mode='x_small',             # support ["xx_small", "x_small", "small"] as shown in paper
        patch_size=(2, 2)
    )
    x = torch.randn(2, 3, 512, 512).cuda()

    ckpt = torch.load('E:\python\python36\SCD\LWSCDNet\weight\MobileViTv3-v1\mobilevitv3_XS_voc_e50_7877\checkpoint_ema_best.pt')
